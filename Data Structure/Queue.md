...
- `Dequeue` 연산을 할 때마다 <b>1)앞으로 요소를 당기는 방식</b>과 <b>2)`Front` 포인터만 증가시키는 방식</b>이 있음
- 하지만, 이 두 방식은 각각 <b>`Queue Shifting`</b>과 <b>`Memory Waste`</b> 문제를 야기  

> <b>❌배열 기반 큐의 문제점</b>  
> ![배열 기반 큐의 문제점 이미지](/Resources/Images/배열%20기반%20큐의%20문제점.png)  
> <b>1. 큐의 이동(Queue Shifting)</b>: 요소가 `dequeue`될 때마다 모든 요소를 앞으로 이동시켜야 한다. 👉 성능 오버헤드 심각  
